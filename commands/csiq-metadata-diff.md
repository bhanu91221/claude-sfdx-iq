...
5. **Conflict detection**
   ...
   - Suggest running `/csiq-retrieve` before deploying if drift is detected
...
```
/csiq-metadata-diff
/csiq-metadata-diff --mode branch --branch develop
/csiq-metadata-diff --target-org staging-sandbox --metadata-type ApexClass
```

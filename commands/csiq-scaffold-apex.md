...
1. **Gather requirements**
   - Ask for the class type / pattern:
     ...
     - **Batch**: `Database.Batchable` implementation (delegates to `/csiq-scaffold-batch` for full scaffold)
...
- For Batch type, suggest using `/csiq-scaffold-batch` for the full batch scaffold with scheduler

## Example Usage

```
/csiq-scaffold-apex
/csiq-scaffold-apex --type Service --name AccountMerge --object Account
/csiq-scaffold-apex --type Selector --name OpportunitySelector --object Opportunity
/csiq-scaffold-apex --type Controller --name ContactSearch
/csiq-scaffold-apex --type Queueable --name LeadAssignment --object Lead
```
